...
passport.use(new GoogleStrategy({
    ...
    callbackURL: "https://gmanager28.herokuapp.com/profile"
  },
  async function(accessToken, refreshToken, profile, done) {
    user.findOne({googleId : profile.id}).then((currentUser) => {
        if(currentUser){
            user.findOneAndUpdate({googleId : profile.id},{access_token: accessToken},{new : true}).then((updatedUser) => {
            ...
            new user({
                email : profile._json.email,googleId : profile.id,access_token : accessToken,display_name : profile.displayName
            }).save().then((newUser) => {
...
router.post('/login',passport.authenticate('google', { scope: ['profile','https://www.googleapis.com/auth/gmail.readonly','email'] }))

router.get('/profile', passport.authenticate('google', { failureRedirect: '/' }),
function(req, res) {
...
router.get('/userprofile',isLoggedIn,asyncError(async (req,res,next)=>{
    const userFind = await user.findOne({email : req.user.email});
    
    res.render('profile',{userFind}); 
}))

router.get('/:id/fill',isLoggedIn,asyncError(async (req,res,next)=>{
    const { id } = req.params;
    const userFind = await user.findById(id);
    res.render('formfill',{userFind});
}))

router.post('/:id/fill',isLoggedIn,asyncError(async (req,res,next)=>{
    ...
    const userFind = await user.findById(req.user.id);
        ...
        if(resultname && resultemail){
            for(input in req.body) {
                    userFind.input.push({searchEmail : req.body[input].searchEmail , name: req.body[input].name});
                    }
                    userFind.save((err)=>{
                        if(err){
...
router.get('/:id/allfolder',isLoggedIn,asyncError(async (req,res,next)=>{
    const userFind = await user.findById(req.params.id);
    res.render('allfolder',{userFind});
}))

router.get('/getprofile',isLoggedIn,asyncError(async (req,res,next)=>{
    const doc = await user.find({email : req.user.email})
    const access_token = doc[0].access_token;
    const apires = await axios.get(`https://gmail.googleapis.com/gmail/v1/users/me/profile?access_token=${access_token}`)
...
router.post('/search',isLoggedIn,asyncError(async (req,res,next)=>{
        ...
        const promises = [];
        var i;
    const apires = await axios.get(`https://gmail.googleapis.com/gmail/v1/users/me/messages?q=from:${searchemail}%20has:attachment&access_token=${access_token}`);
    ...
    for(i=0;i<apires.data.messages.length;i++){
        ...
        //trying promises
         promises[i] = axios.get(`https://gmail.googleapis.com/gmail/v1/users/me/messages/${message_id}?access_token=${access_token}`)
...
router.get('/search',isLoggedIn,asyncError(async (req,res,next)=>{
    const id = req.user.id;
    const userFind = await user.findById(id);
    res.render('search',{userFind})
}))
router.post('/:id/sendme',isLoggedIn,asyncError(async (req,res,next)=>{
        const {id} = req.params;
         const userdata  = await user.findById(id);
        const access_token = userdata.access_token;
    const response = await axios(`https://gmail.googleapis.com/gmail/v1/users/me/messages?access_token=${access_token}`);
    const promises = [];
    const nextPageToken = response.data.nextPageToken;
    response.data.messages.forEach((message,index) => {
        promises[index] = axios.get(`https://gmail.googleapis.com/gmail/v1/users/me/messages/${message.id}?access_token=${access_token}&fields=payload%2Fheaders`)
    })
    ...
    Promise.all(promises).then(resolve =>{
        ...
        function onlyUnique(value, index, self) {
          ...
          uniqueEmail.forEach(email => {
              ...
              if(email.indexOf('<')!= -1){
              respondEmail.push(email.substring(email.indexOf("<")+1,email.indexOf(">")))
              }else{
...
//getting next page
router.post('/:id/nextpage',isLoggedIn,asyncError(async (req,res,next)=>{
        let {nextPageToken} = req.body;
        const {id} = req.params;
         const userdata  = await user.findById(id);
        const access_token = userdata.access_token;
    const response = await axios(`https://gmail.googleapis.com/gmail/v1/users/me/messages?access_token=${access_token}&pageToken=${nextPageToken}`);
    const promises = [];
...
var folderName = null;
router.post('/:id/search/:email',isLoggedIn,asyncError(async (req,res,next)=>{
...
router.get('/:id/:search/pdf',isLoggedIn,asyncError(async (req,res,next)=>{
...
router.post('/finddata',isLoggedIn,asyncError(async (req,res,next)=>{
    const doc = await user.find({email : req.user.email})
        const access_token = doc[0].access_token;
   const { attachmentid, messageid, filename } = req.body;
   const datares = await axios.get(`https://gmail.googleapis.com/gmail/v1/users/me/messages/${messageid}/attachments/${attachmentid}?access_token=${access_token}`)
...
router.get('/:userid/:inputid/edit',isLoggedIn,asyncError(async (req,res,next)=>{
...
//delete request
router.delete('/:id/:inputid/delete',asyncError(async (req,res,next)=>{
...
router.put('/:inputid/edit',asyncError(async(req,res,next)=>{
        //joi validate
        joiValidate(req);
...
router.get('/:id/signout',asyncError(async (req,res,next)=>{
...
router.get('/google81d34312975a22ba.html',(req,res)=>{
    res.sendFile('google81d34312975a22ba.html', { root: path.join(__dirname, '../public') });
